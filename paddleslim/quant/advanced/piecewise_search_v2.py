...
class PieceWiseSearchV2():
    def __init__(self,
        ...
        '''
        PieceWiseSearchV2 provides to search k_piece, alpha and scale.
        compare with PieceWiseSearch: PieceWiseSearchV2 can control the weight scale after search by `max_weight`
    ...
    def search(self, layer_name, sampled_input, act_abs_max, weight):
        ...
        for k_idx, k_piece in enumerate(piece_range):
            ...
            for i in range(len(centroids)):
                ...
                while alpha <= alpha_max:
                    ...
                    if smooth_scale_out is not None:
                        ...
                        scale1 = w_abs_max.reshape(smooth_scale_tmp.shape) * smooth_scale_tmp
                        scale1 = scale1.reshape(smooth_scale_tmp.shape).cast('float32')
                        scale2 = paddle.clip(scale1, max=self.max_weight)
                        re_scale = (scale2+1e-18) / (scale1+1e-18)
                        re_scale = re_scale.cast(dtype)
                        smooth_scale = smooth_scale * re_scale
                        smooth_scale_ = smooth_scale_out + smooth_scale
                        smooth_scale_tmp = smooth_scale_ + mask_for_ones
                        del re_scale, scale2, scale1
                    else:
            ...
            if calibration_loss < global_loss:
                ...
                max_val = (weight*best_scale.reshape(w_abs_max.shape)).abs().max().cast('float32').tolist()
                if self.search_piece:
                    print('Find Better K-Piece {}, max val: {:.2f}'.format(k_piece, max_val))
            if not self.search_piece:
                break
        return best_scale
